...
def thread_list_view(request):
    threads = Thread.objects.by_user(request.user).select_related('first_person', 'second_person').order_by('-updated_at')
...
def chat_message_list_view(request, user_id):
    ...
    messages = ChatMessage.objects.filter(thread=thread).select_related('user').order_by('-timestamp')
